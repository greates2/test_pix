...
def index_find(pattern, string, ignore_case, invert=False):
...
def get_match(pattern, string, color, ignore_case, invert):
    ...
    if color:
        ...
        index = index_find(pattern, string, ignore_case, invert)
...
def grep_file(filename, pattern, color, ignore_case, print_headers, invert,
    ...
    lineno = 1
    while line:
        result = get_match(pattern, line, color, ignore_case, invert)
        if len(result) > 0:
...
def grep_files(paths, pattern, recurse, color, ignore_case, invert, print_headers,
                print_lineno, print_line):
    """Search files and directories."""
    for path in paths:
        if os.path.isfile(path) or path == '(standard input)':
            grep_file(path, pattern, color, ignore_case, print_headers, invert,
                        print_lineno, print_line)
        else:
            if recurse:
                more_paths = [path + '/' + child for child in os.listdir(path)]
                grep_files(more_paths, pattern, recurse, color, ignore_case, invert,
...
def setup_parser():
    ...
    parser.add_argument('-f', '--files-with-matches', action='store_true',
                        help='print only filenames with matches')
    parser.add_argument('-n', '--line-number', action='store_true',
                        help='print line numbers, indexed from 1')
    parser.add_argument('-R', '-r', '--recursive', action='store_true',
                        help='recursively search directories')
    parser.add_argument('-V', '-v', '--invert', action='store_true',
                        help='invert result')

    return parser


DEFAULT_PRINT_OPTIONS = (False, True, True)
...
def main():
    ...
    grep_files(files, pattern, args.recursive, args.color, args.ignore_case, args.invert,
